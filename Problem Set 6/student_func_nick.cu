...
__global__ void detectBorderAndInterior(const uchar4* const d_sourceImg, const size_t numRowsSource, const size_t numColsSource, unsigned char* border, unsigned char* interior, int* xcoords, int* ycoords) {
	...
	if (!belongsToMask(sh_arr[targetposy*(blockDim.x + filterWidth - 1) + targetposx-1]) || 
        !belongsToMask(sh_arr[targetposy*(blockDim.x + filterWidth - 1) + targetposx+1]) ||
		!belongsToMask(sh_arr[(targetposy-1)*(blockDim.x + filterWidth - 1) + targetposx]) ||
        !belongsToMask(sh_arr[(targetposy+1)*(blockDim.x + filterWidth - 1) + targetposx])) {
...
*/
	...
	// to_print = (unsigned char *) malloc(numRowsSource*numColsSource * sizeof(unsigned char));
	// checkCudaErrors(cudaMemcpy(to_print, interior, numRowsSource*numColsSource * sizeof(unsigned char), cudaMemcpyDeviceToHost));

    // std::cout<<"DEBUGGING NICK : interior "<<numRowsSource*numColsSource<<std::endl;
    // for (long unsigned int i=0; i < numRowsSource*numColsSource; i++){
    //     if (i==86269 || i==86270 || i==86271)
    //         std::cout<<(int)to_print[i]<<", ";
    //     // i += 1000;
    // }
    // std::cout<<std::endl;
    // std::cout<<std::endl;
	...
	*/
    ...
    std::cout<<"DEBUGGING GREEN NICK : interior "<<numRowsSource*numColsSource<<std::endl;
    for (long unsigned int i=0; i < numRowsSource*numColsSource; i++){
    ...
    std::cout<<"DEBUGGING NICK : red "<<numRowsSource*numColsSource<<std::endl;
    for (long unsigned int i=0; i < numRowsSource*numColsSource; i++){
        if (i==86268 || i==86269 || i==86270 || i==86271 || i==86272)
