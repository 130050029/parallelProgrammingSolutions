...
{ 
  //PUT YOUR SORT HERE
	...
	for (size_t i = 0; i < 32; i++) {
        ...
        cout<<"REFERENCE CALCULATION FOR BITBLOCK: "<<i<<" "<<endl; // ADDED BY ANKIT
        cout<<"TOTAL_SUM for DIGIT : 1 is :"<<number_trues<<endl;
        ...
        //ADDED BY ANKIT
        cout<<"OFFSET for DIGIT : 1 is :"<<number_falses<<endl;
        unsigned int* h_scan_per_digit;
		h_scan_per_digit = (unsigned int *) malloc(numElems * sizeof(unsigned int *));
		checkCudaErrors(cudaMemcpy(h_scan_per_digit, d_scan_true, numElems * sizeof(unsigned int), cudaMemcpyDeviceToHost));
		cout<<"Output Scanned Address : "<<endl;
		for (int k=0; k< numElems; k+=1000){
			cout<<h_scan_per_digit[k]<<" ";
		}
		cout<<endl;
        //ADDED BY ANKIT
