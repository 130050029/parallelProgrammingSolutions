                       ...
                       int numRows, int numCols)
{
  ...
  if (col < numCols && row < numRows) {
    ...
    //float grey = 0.299f * float(color.x) + 0.587f * float(color.y) + 0.114f * float(color.z);
    //float grey = 0.299f * color.x + 0.587f * color.y + 0.114f * color.z;
    //greyImage[index] = static_cast(grey);
    //greyImage[index] = (unsigned char)grey;
    
    float tempSum = __fadd_rn(__fmul_rn(.299f , color.x), __fmul_rn(.587f , color.y));
    float channelSum = __fadd_rn(tempSum, __fmul_rn(.114f , color.z));
